...
import { ChatBubbleLeftRightIcon, PlusIcon, Cog6ToothIcon, TrashIcon, ArrowPathIcon, PaperAirplaneIcon, UserCircleIcon } from "@heroicons/react/24/outline";
import type { ChatMessage, ChatSession, UserProfile } from "@/types/chat";
import { makeTitleFromFirstUserMessage } from "@/lib/ai/prompt";
...
export default function ChatPage() {
    ...
    setSessions((prev) =>
      prev.map((s) =>
        s.id === active.id
          ? {
              ...s,
              title: s.messages.length === 0 ? makeTitleFromFirstUserMessage(text) : s.title,
              updatedAt: Date.now(),
    ...
    try {
      ...
      while (!done) {
        ...
        if (value) {
          ...
          fullText += chunk;
          const textSoFar = fullText; // maintain accumulating buffer
          setSessions((prev) =>
            prev.map((s) => {
      ...
      const errText = (e as Error).message || "오류가 발생했습니다.";
      setSessions((prev) =>
        prev.map((s) =>
          s.id === active.id
            ? {
                ...s,
                updatedAt: Date.now(),
                messages: s.messages.map((m) => (m.id === asstMsg.id ? { ...m, content: `에러: ${errText}` } : m)),
              }
            : s
        )
      );
  ...
  // Optional server-sync hook (placeholder): fetch server profile if any
  useEffect(() => {
    let ignore = false;
    ...
    <div className="flex h-[calc(100vh-64px)] w-full bg-gradient-to-b from-slate-50 to-white">
      ...
      <aside className="hidden md:flex w-72 flex-col border-r border-slate-200 bg-white/70 backdrop-blur">
        ...
        <div className="flex-1 overflow-y-auto">
          {sessions.length === 0 ? (
            <div className="p-6 text-sm text-slate-500">아직 상담 기록이 없습니다.</div>
          ) : (
            <ul className="p-2">
              {sessions.map((s) => (
                <li
        ...
        <div className="border-t border-slate-200 p-3">
          <button
            className="w-full flex items-center justify-center gap-2 rounded-md border border-slate-300 px-3 py-2 text-sm text-slate-700 hover:bg-slate-50"
      ...
      <main className="flex-1 flex flex-col">
        ...
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-200 bg-white/70 backdrop-blur px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              onClick={newChat}
            >
              <PlusIcon className="h-4 w-4" /> 새 상담
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 md:px-8 py-6">
          ...
          <div className="mx-auto max-w-3xl mb-4 text-xs text-slate-500">
            본 서비스의 상담은 의학적 진단이나 치료를 대체하지 않습니다. 복용 전 의사 또는 약사와 상담하세요.
          </div>
          ...
          <div className="mx-auto max-w-3xl mb-4">
            <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800 flex items-center justify-between gap-3">
              <div>
                {profile ? (
                  <span>프로필: {profile.age ? `${profile.age}세` : "나이 미입력"} · {profile.sex || "성별 미입력"} · 목표 {profile.goals?.join(", ") || "미입력"}</span>
                ) : (
                  <span>맞춤 상담을 위해 개인 프로필을 설정해주세요.</span>
                )}
              </div>
              <button className="rounded-md border border-amber-300 bg-white/70 px-2 py-1 hover:bg-white" onClick={() => setShowSettings(true)}>
                설정하기
              </button>
            </div>
          </div>
          ...
          <div className="mx-auto max-w-3xl space-y-4">
            {!active || active.messages.length === 0 ? (
              <EmptyState onTryExamples={(q) => setInput(q)} />
            ) : (
          ...
          <div className="mx-auto max-w-3xl">
            <div className="flex items-end gap-2">
              <textarea
                ...
                placeholder="예: 위장이 예민한데 오메가-3 복용해도 될까요?"
                value={input}
                rows={1}
      ...
      <div className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-relaxed shadow ${isUser ? "bg-slate-900 text-white" : "bg-white border border-slate-200 text-slate-800"}`}>
        {content || (
          <span className="inline-flex items-center gap-2 text-slate-500">
            <ArrowPathIcon className="h-4 w-4 animate-spin" /> 작성중…
          </span>
        )}
      </div>
    </div>
...
function EmptyState({ onTryExamples }: { onTryExamples: (q: string) => void }) {
  const examples = [
    "수면의 질을 높이려면 어떤 성분이 좋을까요?",
    "철분제와 비타민 C를 같이 먹어도 되나요?",
    "고혈압 약 먹는데 오메가-3 복용해도 안전한가요?",
    "운동 후 회복에 도움 되는 영양제를 추천해주세요.",
  ];
  return (
    <div className="text-center text-slate-600">
      <h2 className="text-xl font-semibold mb-2">건강기능식품 상담을 시작해보세요</h2>
      <p className="text-sm mb-4">프로필을 설정하면 더 정확한 맞춤 추천이 가능해요.</p>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="w-full max-w-2xl rounded-2xl bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <div className="font-semibold text-slate-800">상담 프로필</div>
        ...
        <div className="px-6 py-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <LabeledSelect label="성별" value={local.sex || ""} onChange={(v) => set("sex", v as any)} options={[{ label: "선택안함", value: "" }, { label: "남성", value: "male" }, { label: "여성", value: "female" }, { label: "기타", value: "other" }]} />
          ...
          <LabeledChips label="복용중인 약" placeholder="예: 메트포르민" values={local.medications || []} onChange={(vals) => set("medications", vals)} />
          <LabeledChips label="질환/증상" placeholder="예: 고혈압" values={local.conditions || []} onChange={(vals) => set("conditions", vals)} />
          <LabeledChips label="알레르기" placeholder="예: 해산물" values={local.allergies || []} onChange={(vals) => set("allergies", vals)} />
          <LabeledChips label="목표" placeholder="예: 수면, 면역" values={local.goals || []} onChange={(vals) => set("goals", vals)} />
          <LabeledChips label="식이 제한" placeholder="예: 비건, 글루텐프리" values={local.dietaryRestrictions || []} onChange={(vals) => set("dietaryRestrictions", vals)} />
          <LabeledSelect label="임신/수유" value={local.pregnantOrBreastfeeding ? "yes" : local.pregnantOrBreastfeeding === false ? "no" : ""} onChange={(v) => set("pregnantOrBreastfeeding", v === "yes" ? true : v === "no" ? false : undefined)} options={[{ label: "선택안함", value: "" }, { label: "예", value: "yes" }, { label: "아니오", value: "no" }]} />
          <LabeledSelect label="카페인 민감" value={local.caffeineSensitivity ? "yes" : local.caffeineSensitivity === false ? "no" : ""} onChange={(v) => set("caffeineSensitivity", v === "yes" ? true : v === "no" ? false : undefined)} options={[{ label: "선택안함", value: "" }, { label: "예", value: "yes" }, { label: "아니오", value: "no" }]} />
        </div>
        <div className="flex items-center justify-between border-t border-slate-200 px-6 py-4">
          ...
          >프로필 초기화</button>
          <div className="flex items-center gap-2">
            <button className="rounded-md border border-slate-300 px-4 py-2 text-sm text-slate-700 hover:bg-slate-50" onClick={onClose}>취소</button>
  ...
  return (
    <div className="text-sm">
      ...
      <div className="flex items-center gap-2">
        ...
        />
        <button className="rounded-md border border-slate-300 bg-white px-3 py-2 text-slate-700 hover:bg-slate-50" onClick={addChip}>추가</button>
      </div>
    </div>
  );
}
