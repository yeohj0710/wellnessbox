...
const ACTION_HINT_REGEX =
  /(담아|추가|장바구니|카트|비워|초기화|구매|결제|주문|열어|보여|프로필|설정|내\s*주문|주문\s*조회|내\s*정보|내\s*데이터|문의|고객센터|연락처|약관|개인정보|환불|이메일|전화|7일치|홈\s*상품|상품\s*섹션|진단|검사|빠른\s*검사|정밀\s*검사|체크\s*ai|check\s*ai|자가\s*진단|이동|가자|가줘|둘러봐|탐색|대화로|채팅으로|약국\s*관리|라이더|관리자)/i;
