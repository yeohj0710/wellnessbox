...
function buildCategoryAliases(rawCategory: string) {
  ...
  const compact = normalized.replace(/건강|관리|기능|제품|추천/g, "").trim();
...
export function shouldAllowCategoryFallback(line: RecommendationLine) {
  return (
    isPlaceholderProductName(line.productName, PLACEHOLDER_PRODUCT_NAME_SET) ||
    isCategoryLikeProductName(line.productName, line.category, CATEGORY_KEYWORD_SET)
  );
}
