        ...
        <div className={styles.editorPanelBody}>
          <div className={styles.editorGuide}>
            ...
            <ul className={styles.editorGuideList}>
              <li>레이아웃 검증을 실행하면 페이지 넘침, 경계선, 배치 이슈를 먼저 점검해 줍니다.</li>
              <li>A4 미리보기를 열면 실제 출력에 가까운 화면으로 확인할 수 있어요.</li>
              <li>문제가 있으면 수정 후 다시 검증한 뒤 PDF를 내려받아 주세요.</li>
            </ul>
          </div>
          ...
          <div className={styles.actionRow}>
            ...
            >
              {busy ? (
                <InlineSpinnerLabel label="레이아웃 검증 중" />
              ) : (
                "레이아웃 검증 실행"
              )}
            </button>
            ...
            >
              {showExportPreview ? "A4 미리보기 닫기" : "A4 미리보기 보기"}
            </button>
          </div>
            ...
            <section className={styles.editorSection}>
              <div className={styles.editorSectionHead}>
                ...
                <p className={styles.editorSectionHint}>
                  총 {validationAudit?.validation?.length ?? 0}개 단계
                </p>
              </div>
                ...
                {(validationAudit?.validation ?? []).map((entry, index) => (
                  ...
                  >
                    ...
                    <p className={styles.editorStatusMeta}>
                      static {entry.staticIssueCount ?? 0} / runtime{" "}
                      {entry.runtimeIssueCount ?? 0} / 중복제거{" "}
                      {entry.dedupedIssueCount ?? entry.issues?.length ?? 0}
                    </p>
                  </article>
                ))}
              </div>
