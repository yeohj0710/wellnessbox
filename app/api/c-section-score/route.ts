...
export async function POST(req: Request) {
  ...
  if (!Array.isArray(cats) || !Array.isArray(answers))
    return NextResponse.json({ error: '형식이 올바르지 않습니다.' }, { status: 400 });
  if (cats.length !== 3 || answers.length !== 3)
    return NextResponse.json({ error: '입력 개수가 올바르지 않습니다.' }, { status: 400 });
  ...
  for (const row of answers) {
    if (!Array.isArray(row) || row.length !== 5)
      return NextResponse.json({ error: '응답 형식이 잘못되었습니다.' }, { status: 400 });
    for (const v of row) {
      if (typeof v !== 'number' || v < 0 || v > 3)
        return NextResponse.json({ error: '응답 값이 범위를 벗어났습니다.' }, { status: 400 });
    }
  }
  try {
