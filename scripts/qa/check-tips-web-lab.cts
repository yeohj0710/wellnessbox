...
const { canRunTipsLabAction, isTipsLabTransitionAllowed } = require("../../lib/server/tips-lab/state");
...
const stylesUi = read("components/tips/interim.module.css");
assert.match(consoleUi, /activeStage === 0[\s\S]*initializeEvaluationSession/);
assert.match(consoleUi, /activeStage === 1[\s\S]*blindTestRef\.current\?\.verifyAll/);
assert.match(consoleUi, /activeStage === 4[\s\S]*execute\("retrieve_evidence"/);
assert.match(consoleUi, /ref=\{blindTestRef\}/);
assert.match(consoleUi, /stageModalBackdrop/);
assert.match(consoleUi, /stageModalContent/);
assert.match(consoleUi, /평가 단계 닫기/);
assert.match(consoleUi, /if \(bounded === 0 && !closingModal\)/);
const blindTestHandleUi = read("components/tips/BlindTestExplorer.tsx");
assert.match(blindTestHandleUi, /export type BlindTestExplorerHandle/);
assert.match(blindTestHandleUi, /useImperativeHandle\(ref/);
...
for (const token of ["평가 시나리오", "전체 과정 실행", "DEMO_SCENARIOS", "runScenario", "추천부터 추적 기록까지 자동 실행"]) assert.match(consoleUi, new RegExp(token));
...
const datasetWorkbenchUi = read("components/tips/DatasetEvaluationWorkbench.tsx");
assert.match(workflowMapUi, /workflowNodeModal/);
assert.doesNotMatch(workflowMapUi, /status:"데이터 준비 완료"/);
assert.match(workflowMapUi, /영속 저장 미확인/);
assert.match(route, /appendTipsLabEvent/);
assert.match(read("prisma/schema.prisma"), /model TipsLabSession/);
assert.match(read("prisma/schema.prisma"), /model TipsLabEvent/);
...
assert.match(workflowMapUi, /현재 실행 상태/);
assert.doesNotMatch(workflowMapUi, /transform="scale/);
...
assert.equal(canRunTipsLabAction("CANDIDATES_READY", "recommend"), false);
assert.equal(canRunTipsLabAction("CANDIDATES_READY", "retrieve_evidence"), true);
assert.equal(canRunTipsLabAction("ACTIVE_PLAN", "ingest_pro"), true);
assert.equal(canRunTipsLabAction("NEW", "ingest_pro"), false);
assert.equal(isTipsLabTransitionAllowed("NEW", "SAFETY_REVIEW"), true);
assert.equal(isTipsLabTransitionAllowed("NEW", "ACTIVE_PLAN"), false);
assert.equal(isTipsLabTransitionAllowed("ESCALATED", "ACTIVE_PLAN"), false);
...
assert.match(consoleUi, /EVALUATION_STAGES/);
assert.match(consoleUi, /evaluatorDock/);
assert.match(consoleUi, /자세히 설명/);
assert.match(consoleUi, />입력</);
assert.match(consoleUi, />계산</);
...
for (const token of ["연결된 성과지표", "평가 열기", "evaluation", "kpis"]) assert.match(workflowMapUi, new RegExp(token));
...
for (const token of ['href="/assess"', 'href="/explore"', 'href="/my-orders"', 'href="/chat"', "후속평가·배합 조정"]) assert.match(workflowMapUi, new RegExp(token));
...
for (const token of ["저장 세션 재생 검증", "저장 세션 다시 조회", "선택 세션 재생", "동일 결과 확인", "버전 불일치로 재생 중단"]) assert.match(consoleUi, new RegExp(token));
assert.match(runtimeModule, /list_rnd_sessions/);
assert.match(runtimeModule, /replay_rnd_session/);
assert.match(consoleUi, /rndReplay\.executionId===selectedRndExecutionId/);
assert.match(consoleUi, /if \(action === "list_rnd_sessions"\) setRndReplay\(null\)/);
assert.doesNotMatch(consoleUi, /expected_output_sha256|actual_output_sha256|request_json/);
...
assert.match(proStudyUi, /공인 설문 점수로 개선도를 계산합니다/);
assert.match(proStudyUi, /120명 예시 데이터 평가/);
assert.match(proStudyUi, /CSV 저장/);
assert.match(proStudyUi, /입력값 수정/);
assert.match(proStudyUi, /setStep/);
...
assert.match(proStudyUi, /PSS10/);
assert.match(proStudyUi, /평균 개선도/);
assert.match(proStudyUi, /다른 테스터 평가/);
assert.match(proStudyUi, /useImperativeHandle/);
assert.match(proStudyUi, /ProStudySimulationHandle/);
assert.doesNotMatch(proStudyUi, /className=\{styles\.primaryButton\}[^>]*>다음/);
assert.doesNotMatch(proStudyUi, /계획서 19·25·26쪽/);
...
assert.match(blindTestUi, /className=\{styles\.explanationInline\}/);
assert.doesNotMatch(blindTestUi, /explanationOverlay/);
assert.match(blindTestUi, /현재 모델이 계산한 추천/);
for (const token of ["입력값 수정", "수정 입력으로 재추론", "수정 전 현재 모델이 계산한 추천", "수정 후 모델 추천", "recompute_blind_test"]) assert.match(blindTestUi, new RegExp(token));
assert.match(runtimeModule, /recomputeBlindTest/);
...
assert.match(researchOverviewUi, /기술 블록과 연결된 연구 데이터·성과지표/);
for (const token of ["학습·평가 데이터 원문과 전체 재채점", "평가 데이터 30,000건 재채점", "verify_dataset_split", "list_dataset_cases", "TRAIN_RESUBSTITUTION"]) assert.match(`${datasetWorkbenchUi}\n${runtimeModule}\n${read("lib/server/tips-lab/dataset-evaluation.ts")}`, new RegExp(token));
assert.match(researchOverviewUi, /데이터셋 분할 구성/);
assert.match(researchOverviewUi, /성과지표별 평가 실행/);
for (const token of ["7개 KPI 전체 데이터 재평가", "verify_all_kpis", "이번 실행 결과", "재평가 필요", "직전 산출물"]) assert.match(`${researchOverviewUi}\n${runtimeModule}`, new RegExp(token));
assert.match(researchOverviewUi, /측정·평가 방법/);
assert.match(researchOverviewUi, /onNavigate\(path\.stage\)/);
assert.match(researchOverviewUi, /KPI-1의 100\.00%/);
...
assert.equal(decideNextAgentTask({ safetyChecked: true, candidateCount: 2, evidenceRetrieved: true, evidenceStale: true }).selectedTask, "retrieve_evidence");
...
for (const token of ["execute_agent_task", "다음 작업 결정·실행", "주요 시나리오 일괄 평가", "사후조건 충족률", "고위험 오동작", "도구 응답 지연", "근거 유효기간 경과"]) assert.match(agentEvaluatorUi, new RegExp(token));
assert.match(runtimeModule, /DUPLICATE_ACTION/);
assert.match(runtimeModule, /CONSENT_REQUIRED/);
assert.match(runtimeModule, /INVALID_TRANSITION/);
assert.match(runtimeModule, /TIMED_OUT/);
