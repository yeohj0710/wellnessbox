(function runDbEnvCheck() {
  ...
  const { ensurePrismaEnvConfigured } =
  ...
  if (!result.ok) {
    ...
    console.error(
      `[db-env] selected databaseKey=${result.databaseKey ?? "-"} directKey=${
        result.directKey ?? "-"
  ...
  console.log(
    `[db-env] OK databaseKey=${result.databaseKey ?? "-"} directKey=${
      result.directKey ?? "-"
    } conflicts=${conflictSummary}`
  );
})();
