...
- 공용 PC 안전을 위해 `session-sync`/`profile-sync` 요청에서는 병합을 수행하지 않고 AppUser에 이미 연결된 `clientId`를 그대로 사용합니다.

## 우선순위 및 충돌 처리
- AppUser에 기존 `clientId`가 없다면 안전하게 새 값을 발급하며, 병합이 허용된 요청(`kakao-login`/`phone-link`)이고 최근 15분 내 동일 User-Agent로 활동한 브라우저라면 해당 `clientId`를 승계합니다.
- 기존 값과 다른 `clientId`가 들어와도 병합 허용 조건을 만족하지 않으면 AppUser에 저장된 값을 우선 사용합니다.
- 동일한 `clientId`가 다른 AppUser에 이미 연결되어 있으면 교차 노출을 막기 위해 기존 AppUser의 `clientId`를 유지하고 신규 연결은 거부(새로운 `clientId` 발급)합니다.

## 데이터 이관(merge)
- 병합 허용 시점(`kakao-login`/`phone-link`)에만 AppUser의 `clientId` 변경 및 데이터 이동을 수행합니다.
