...
### 4.3 숨겨진 강제 재조회(운영자 전용)

노출 조건:
- 관리자 로그인 상태이거나 URL에 `?debug=1` 플래그가 있을 때만 표시

실행 위치:
- `/employee-report` 진입 후 레포트가 표시된 상태
- `운영자 도구 (비용 발생)` 접기 영역 안의 `강제 재조회 실행` 버튼

실행 절차:
1. `운영자 도구 (비용 발생)` 펼치기
2. `강제 재조회 실행` 클릭
3. 확인 모달에서 체크박스 선택
4. 입력창에 `강제 재조회`를 정확히 입력
5. `강제 재조회 실행` 버튼 최종 클릭

서버 동작:
- `POST /api/b2b/employee/sync`에 `forceRefresh: true`로 전달
- 기본 캐시 경로를 우회하고 외부 재조회 경로를 우선 시도
- 쿨다운 미충족 시 `429` + `code=SYNC_COOLDOWN`으로 차단
