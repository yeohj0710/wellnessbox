...
type WorkflowNodeDetail = {
  title:string; role:string; status:string; receives:string[]; sends:string[]; implementation:string; stage?:number;
};
...
export default function ResearchWorkflowMap(props: WorkflowMapProps) {
  ...
  const details:Record<string,WorkflowNodeDetail>={
    cron:{title:"CronJob",role:"정해진 주기에 평가 세션을 다시 호출합니다.",status:"주기 호출 경로 정의",receives:["평가 주기","활성 계획 식별자"],sends:["재평가 요청","후속평가 기한 신호"],implementation:"자기적응형 AI가 현재 상태를 다시 판단하도록 호출하는 운영 경로입니다.",stage:5},
    consumer:{title:"소비자",role:"진단, 주문, 복용, 대화와 후속평가의 시작점입니다.",status:"서비스 입력 경로 연결",receives:["추천 결과","배송 상태","후속평가 요청"],sends:["건강 조건","대화·복용 피드백","재검사 결과"],implementation:"정밀진단·추천·주문·대화 상담 화면이 실제 서비스 경로로 연결되어 있습니다.",stage:3},
    agent:{title:"자기적응형 AI",role:"상태에 따라 다음 작업과 호출 도구를 결정합니다.",status:agent,receives:["사용자 요청","프로필·근거","PRO·웨어러블·이상사례"],sends:["안전검사 요청","추천·최적화 요청","후속 작업"],implementation:"다음 작업 결정, 허용 상태전이, 중복 방지와 사후조건 검증을 실행합니다.",stage:5},
  ...
  return <section className={`${styles.section} ${styles.workflowHub}`} aria-labelledby="workflow-map-title">
    ...
    {selectedNode&&<div className={styles.helpOverlay} role="presentation" onMouseDown={(event)=>{if(event.target===event.currentTarget)setSelectedNode(null);}}><section className={`${styles.stageHelpModal} ${styles.workflowNodeModal}`} role="dialog" aria-modal="true" aria-labelledby="workflow-node-title"><header><div><span>기술 블록 연결 상태</span><h2 id="workflow-node-title">{selectedNode.title}</h2></div><button type="button" aria-label="모달 닫기" onClick={()=>setSelectedNode(null)}>×</button></header><p className={styles.helpPurpose}>{selectedNode.role}</p><div className={styles.workflowNodeStatus}><span>현재 실행 상태</span><strong>{selectedNode.status}</strong></div><div className={styles.workflowDataGrid}><section><h3>받는 데이터</h3><ul>{selectedNode.receives.map(item=><li key={item}>{item}</li>)}</ul></section><section><h3>보내는 데이터</h3><ul>{selectedNode.sends.map(item=><li key={item}>{item}</li>)}</ul></section></div><div className={styles.workflowImplementation}><span>현재 구현</span><p>{selectedNode.implementation}</p></div><footer><button type="button" onClick={()=>setSelectedNode(null)}>회로로 돌아가기</button>{selectedNode.stage!==undefined&&<button type="button" className={styles.primaryButton} onClick={()=>openStage(selectedNode.stage!)}>해당 검증 실행</button>}</footer></section></div>}
  </section>;
}
