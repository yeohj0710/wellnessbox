...
export default function ProStudySimulation() {
  ...
  return <section className={`${styles.section} ${styles.proStudy}`} aria-labelledby="pro-study-title">
    ...
    {selected&&result&&<div className={styles.studyResult}><h3>3. 개인별 전후 비교</h3><div><span>기초 PRO</span><strong>{result.baselineScore.toFixed(1)}</strong></div><div><span>최종 PRO</span><strong>{result.latestScore?.toFixed(1)??"—"}</strong></div><div><span>변화량</span><strong>{result.change===null?"—":`${result.change>=0?"+":""}${result.change.toFixed(1)}`}</strong></div><div><span>반응자 판정</span><strong>{result.change===null?"미평가":result.responder?"반응":"비반응"}</strong></div></div>}
  </section>;
}
