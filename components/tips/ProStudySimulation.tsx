...
export default function ProStudySimulation() {
  ...
  return <section className={`${styles.section} ${styles.proStudy}`} aria-labelledby="pro-study-title">
    ...
    <div className={styles.studyActions}><button onClick={samples}>PRO 전체 과정 원클릭 시연</button><button onClick={exportCsv} disabled={!participants.length}>원자료 CSV 내보내기</button><button disabled={!selected} onClick={()=>{setParticipants(list=>list.filter(p=>p.id!==selectedId));setSelectedId("");}}>선택 테스터 삭제</button><button onClick={()=>{setParticipants([]);setSelectedId("");}}>전체 초기화</button></div>
