...
import type { OrderSelfServiceCardModel } from "@/lib/order/self-service";

type OrderSelfServiceCardProps = {
  model: OrderSelfServiceCardModel;
...
export function OrderSelfServiceCard({
  model,
  onPrimaryAction,
  onSecondaryAction,
  className = "",
    ...
    <section className={`rounded-3xl border p-4 shadow-sm ${tone.card} ${className}`}>
      ...
      {model.primaryActionLabel || model.secondaryActionLabel ? (
        <div className="mt-4 flex flex-col gap-2 sm:flex-row">
          {model.primaryActionLabel && onPrimaryAction ? (
            <button
              type="button"
  ...
  return (
    <BetaFeatureGate title="Beta 조회 안내" helper="필요할 때만 펼쳐보세요.">
      {content}
    </BetaFeatureGate>
  );
}
