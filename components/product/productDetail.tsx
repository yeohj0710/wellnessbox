...
function FactRow({ label, value }: ProductDetailFactRow) {
  return (
    <div className="grid grid-cols-[112px_minmax(0,1fr)] gap-3 border-b border-slate-100 py-3 last:border-b-0 last:pb-0 first:pt-0">
    ...
    <div className="fixed inset-0 z-20 flex items-center justify-center bg-white">
      ...
      >
        ...
        <div className="relative">
          {images.length > 0 ? (
            <div className="relative h-80 w-full overflow-hidden bg-gradient-to-b from-slate-50 via-white to-slate-50 sm:h-[24rem]">
              {isImageLoading && (
          ...
          <button
            type="button"
            onClick={onClose}
            className="absolute right-4 top-4 grid h-10 w-10 place-items-center rounded-full bg-white/95 shadow ring-1 ring-slate-200 transition hover:bg-slate-50 sm:right-5 sm:top-5"
          >
            <XMarkIcon className="h-5 w-5 text-slate-700" />
          </button>
        </div>

        <div className="px-5 pb-36 pt-6 sm:px-7">
          <div className="flex flex-col gap-5">
            <div>
              <h1 className="text-[22px] font-bold leading-tight text-slate-950 sm:text-[28px]">
                {product.name}
              </h1>
              <p className="mt-2 text-[13px] leading-6 text-slate-500">
                상품명, 옵션, 성분 표기, 포장 단위 기준으로 정리한 제품 정보입니다.
              </p>
            </div>

            <div className="rounded-[28px] border border-slate-200 bg-white p-4 shadow-[0_18px_48px_-36px_rgba(15,23,42,0.4)] sm:p-5">
              <div className="flex flex-col gap-4">
                ...
                {resolvedDetailFacts?.highlights?.length ? (
                  <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                    {resolvedDetailFacts.highlights.map((item) => (
                      <div
            ...
            <div className="space-y-4">
              ...
              <section className="rounded-[28px] border border-slate-200 bg-slate-50 px-5 py-4">
                <div className="space-y-1 text-[13px] leading-6 text-slate-600">
                  <p>주소 입력 후 약국이 선택되면 정확한 상품 가격을 안내해드려요.</p>
          ...
          <div className="fixed bottom-0 left-1/2 flex w-full max-w-[720px] -translate-x-1/2 items-center justify-between gap-4 border-t border-slate-200 bg-white/95 px-5 py-4 backdrop-blur-sm sm:px-7">
            <div>
              ...
              <div className="mt-1 text-[24px] font-extrabold text-sky-600">
                {formatPriceRange({
                  product,
                  quantity,
                  optionType: selectedOption || undefined,
            ...
            <button
              ...
              className="rounded-full bg-sky-500 px-8 py-3 text-[16px] font-semibold text-white shadow-sm transition hover:bg-sky-600"
            >
              담기
            </button>
          </div>
