...
export type FallbackActionDraft = {
  ...
  navigationAction: ChatActionType | null;
  cartMode: ChatAgentCartIntentMode;
};

function pushUnique(out: ChatActionType[], action: ChatActionType) {
  if (!out.includes(action)) out.push(action);
}

export function resolveFallbackActionDraft(input: {
  text: string;
  runtimeFlags: RuntimeContextFlags;
  hasRecommendationContext: boolean;
}): FallbackActionDraft {
  const { text, runtimeFlags, hasRecommendationContext } = input;
  ...
  if (runtimeFlags.inMyOrders && MANUAL_ORDER_LOOKUP_REGEX.test(text)) {
    ...
    runtimeFlags.inMyOrders &&
    LINKED_ORDER_LOOKUP_REGEX.test(text)
  ) {
    pushUnique(actions, "focus_linked_order_lookup");
  }
  ...
  if (
    ...
    pushUnique(actions, "focus_home_products");
  }
  ...
  if (
    ...
    pushUnique(actions, "focus_me_profile");
  }
  ...
  if (
    ...
    pushUnique(actions, "focus_me_orders");
  }
  ...
  if (
    ...
    pushUnique(actions, "focus_my_data_account");
  }
  ...
  if (
    ...
    pushUnique(actions, "focus_my_data_orders");
  }
  ...
  if (
    ...
    pushUnique(actions, "focus_check_ai_form");
  }
  ...
  if (
    ...
    pushUnique(actions, "focus_assess_flow");
  }
  ...
  if (navigationAction) {
    pushUnique(actions, navigationAction);
  }

  if (CLEAR_CART_REGEX.test(text)) {
    pushUnique(actions, "clear_cart");
  }

  if (prefersChatMode && QUICK_CHECK_REGEX.test(text)) {
    pushUnique(actions, "start_chat_quick_check");
  } else if (
    prefersChatMode &&
    (DEEP_ASSESS_REGEX.test(text) || GENERIC_ASSESS_REGEX.test(text))
  ) {
    pushUnique(actions, "start_chat_assess");
  } else if (
    !navigationAction &&
  ...
  return { actions, navigationAction, cartMode };
}

export function hasFallbackActionOrCartIntent(input: FallbackActionDraft) {
  return input.actions.length > 0 || input.cartMode !== "none";
}

export function buildFallbackAssistantReply(input: FallbackActionDraft) {
  const assistantParts: string[] = [];

  if (input.cartMode !== "none") {
    assistantParts.push(
      input.cartMode === "buy_all"
  ...
  const pushFeedback = (action: ChatActionType) => {
    const text = FALLBACK_ACTION_FEEDBACK[action];
    if (!text || pushedMessages.has(text)) return;
    pushedMessages.add(text);
    assistantParts.push(text);
  };

  if (input.navigationAction) {
    pushFeedback(input.navigationAction);
  }

  for (const action of input.actions) {
    if (action === input.navigationAction) continue;
  ...
  return assistantParts.join(" ").trim() || "요청하신 동작을 실행해둘게요.";
}

export function buildFallbackReason(input: FallbackActionDraft) {
  const reasons: string[] = [];

  if (input.cartMode !== "none") reasons.push("cart");
  if (input.navigationAction) reasons.push(`primary:${input.navigationAction}`);
  if (input.actions.includes("focus_home_products")) reasons.push("focus:home-products");
  if (input.actions.includes("focus_manual_order_lookup")) reasons.push("focus:manual-order");
  if (input.actions.includes("focus_linked_order_lookup")) reasons.push("focus:linked-order");
  if (input.actions.includes("focus_me_profile")) reasons.push("focus:me-profile");
  if (input.actions.includes("focus_me_orders")) reasons.push("focus:me-orders");
  if (input.actions.includes("focus_my_data_account")) reasons.push("focus:my-data-account");
  if (input.actions.includes("focus_my_data_orders")) reasons.push("focus:my-data-orders");
  if (input.actions.includes("focus_check_ai_form")) reasons.push("focus:check-ai-form");
  if (input.actions.includes("focus_assess_flow")) reasons.push("focus:assess-flow");
  if (input.actions.includes("clear_cart")) reasons.push("cart-clear");
  if (input.actions.includes("start_chat_quick_check")) reasons.push("chat-quick-check");
  if (input.actions.includes("start_chat_assess")) reasons.push("chat-deep-assess");

  return `fallback: ${reasons.join(",") || "action"}`;
}

export function resolveFallbackSuggestedActions(input: {
  assistantText: string;
  runtimeFlags: RuntimeContextFlags;
}): ChatAgentSuggestedAction[] {
  const { assistantText, runtimeFlags } = input;
  ...
  const hasChatAssessmentContext =
    /(?:대화형\s*(빠른|정밀)?\s*검진\s*문항|질문|chat\s*assessment)/i.test(
      assistantText
    );
