  ...
  return {
    password:
      typeof (body as { password?: unknown })?.password === "string"
        ...
        : "",
    loginType:
      typeof (body as { loginType?: unknown })?.loginType === "string"
        ? (body as { loginType: string }).loginType
        : "",
  };
}

export async function runVerifyPasswordPostRoute(req: Request) {
  let body: unknown;
  try {
    body = await req.json();
  } catch {
  ...
  const { password, loginType } = readLoginBody(body);
