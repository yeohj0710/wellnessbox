...
function buildPdfDocumentTitle(payload: unknown, periodKey: string | null) {
  const reportPayload = payload as ReportSummaryPayload | null | undefined;
  const employeeLabel = normalizeFilenameToken(reportPayload?.meta?.employeeName, "직원");
  const periodLabel = normalizeFilenameToken(reportPayload?.meta?.periodKey, periodKey || "최근");
