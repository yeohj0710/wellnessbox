...
import {
  ...
  DEFAULT_ORDER_QUERY_TAKE,
  getBasicOperatorOrdersPage,
...
export async function getOrdersWithItemsAndStatusPaginated(
  ...
  const result = await getPaginatedOrderSummaries({ phone, password }, page, take);
...
export async function getOrdersWithItemsByPhonePaginated(
  ...
  const result = await getPaginatedOrderSummaries(
    { OR: phoneCandidates.map((candidate) => ({ phone: candidate })) },
    page,
    take
  );
...
export async function getBasicOrdersByRider(
  ...
  return getBasicOperatorOrdersPage(where, page, take);
}
