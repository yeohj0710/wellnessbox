...
async function run() {
  ...
  try {
    ...
    if (!employee?.name || !employee?.birthDate || !employee?.phoneNormalized) {
      throw new Error("employee identity missing");
    }

    const sessionRes = await context.request.post(`${BASE_URL}/api/b2b/employee/session`, {
      failOnStatusCode: false,
      data: {
        name: employee.name,
    ...
    if (sessionRes.status() !== 200) {
      throw new Error(`employee session failed: ${sessionRes.status()}`);
    }

    await page.goto("/employee-report?debug=1", { waitUntil: "networkidle", timeout: 120000 });
    await page.waitForTimeout(4000);
    const employeeShot = path.join(ROOT, "tmp", "employee-report-redesign-latest-v4.png");
    ...
    const adminShot = path.join(ROOT, "tmp", "admin-b2b-reports-redesign-latest-v4.png");
